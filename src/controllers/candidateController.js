...
async function getCandidateDetail(req, res) {
  const candidate = await prisma.candidate.findUnique({
    where: {
      candidate_slug: req.params.candidate_name
        .toLowerCase()
        .replace(/\s+/g, "-"),
    },
  });
...
async function storeCandidate(req, res) {
  const candidatesSchema = z.object({
    ...
    candidate_biography: z
      .string()
      .min(1, { message: "Candidate biography is required" }),
    candidate_vision: z
      .string()
      .min(1, { message: "Candidate vision is required" }),
    candidate_mission: z
      .string()
      .min(1, { message: "Candidate mission is required" }),
  });
  ...
  const {
    ...
    candidate_vision,
    candidate_mission,
  } = candidatesSchema.parse(req.body);
  ...
  if (isUniqueCandidate)
    throw new ValidationError("candidate is already exists");
...
async function updateCandidate(req, res) {
  ...
  const candidateSlug = candidate_name.toLowerCase().replace(/\s+/g, "-");
